...
class MessageRepository {
  Future<Either<String, Message?>> sendMessage(Message message) async {
    try {
      final request = ModelMutations.create(message, authorizationMode: APIAuthorizationType.userPools);
  ...
  Future<Either<String, List<Message>>> getLatestMessages() async {
    const graphQLDocument = '''query GetLatestMessages {
            messagesByDate(type: "Message" sortDirection: DESC limit: 10) {
              items {
                id
                message
                username
    ...
    try {
      final response = await Amplify.API
          .query(request: GraphQLRequest<String>(document: graphQLDocument, authorizationMode: APIAuthorizationType.userPools ))
          .response;
  ...
  Stream<GraphQLResponse<Message>> subscribeToMessages() {
    final subscriptionRequest = ModelSubscriptions.onCreate(Message.classType, authorizationMode: APIAuthorizationType.userPools);
